...
def dashboard() -> str:
    try:
        ...
        # Determine which stations to show based on filter
        if grupo in ("northface", "sanmina"):
            group_rows = balanceo.get(grupo.upper(), [])
            group_stations = []
            for r in group_rows:
            ...
            group_stations = []
            for line_name in ("NORTHFACE", "SANMINA"):
                ...
                for r in line_rows:
                    ...
                    if ct > 0:
                        group_stations.append(
                            Station(
                                station_id=f"{line_name[:2]}-{r.get('estacion', '')}",
                                station_name=f"{line_name[:2]} {r.get('estacion', '')}",
                                time_seconds=ct,
                                operators=1,
                                observations="",
                                action="",
        ...
        # Group plan actions by line for display
        plan_by_line: dict[str, list[dict]] = {}
        for p in plan:
            line = str(p.get("linea", "OTRO")).strip().upper() or "OTRO"
            plan_by_line.setdefault(line, []).append(p)
        # Sort lines for consistent display
        plan_by_line = dict(sorted(plan_by_line.items()))
