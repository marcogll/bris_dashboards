...
@st.cache_data(ttl=60)
def load_demanda():
    rows = read_csv(CURATED_DIR / "demanda_afl.csv")
    if not rows:
...
# ──────────────────────────────────────────────
    ...
    if not bal_df.empty and "linea" in bal_df.columns:
        ...
        st.info("Sin datos de balanceo. Verifica `adriana_projects/data/curated/balanceo_lineas.csv`")
